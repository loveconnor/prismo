...
import { lucideCode, lucideAtom, lucideDatabase, lucideShield, lucideBookOpen, lucideCalculator, lucideFileText, lucideTreePine, lucideNetwork, lucideGlobe } from '@ng-icons/lucide';
    ...
    <section>
      <div class="mb-4 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <ng-icon name="lucideGlobe" class="h-6 w-6 text-blue-500"></ng-icon>
          <h2 class="text-2xl font-semibold text-foreground">Public Labs</h2>
        </div>
        <app-button variant="outline" (click)="viewAllLabs()">View All</app-button>
      </div>
      ...
      <div *ngIf="!loading && !error && labs.length > 0" class="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 items-stretch auto-rows-fr">
        <ng-container *ngFor="let lab of labs; let i = index">
          <app-card
            className="group relative grid grid-rows-[auto_1fr_auto] h-full shadow-none hover:shadow-sm transition-colors hover:bg-white/5 focus-within:ring-1 focus-within:ring-blue-500/30"
          >
            <app-card-header className="pb-0">
              <div class="flex h-11 w-11 items-center justify-center rounded-lg bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400">
            ...
            <app-card-content className="flex-1 flex flex-col gap-3 pt-4">
              ...
              <div class="flex flex-wrap gap-1">
                <span *ngFor="let skill of lab.skills.slice(0, 3)" 
                      class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200">
                  {{ skill }}
                </span>
                <span *ngIf="lab.skills.length > 3" 
                      class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200">
                  +{{ lab.skills.length - 3 }}
                </span>
              </div>
      ...
      <div *ngIf="!loading && !error && labs.length === 0" class="text-center py-8">
        <ng-icon name="lucideGlobe" class="h-12 w-12 text-muted-foreground mx-auto mb-4"></ng-icon>
        <p class="text-muted-foreground">No public labs available at the moment.</p>
      </div>
    </section>
  `
})
export class PublicLabsComponent implements OnInit, OnDestroy {
  ...
  loadLabs() {
    ...
    this.labsService.getPublicLabs()
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (labs) => {
          // Limit to 8 labs for the home page
          this.labs = labs.slice(0, 8);
          this.loading = false;
          this.cdr.detectChanges();
        },
        error: (error) => {
          console.error('Error loading public labs:', error);
          this.error = 'Failed to load public labs';
          this.loading = false;
        }
      });
  }
